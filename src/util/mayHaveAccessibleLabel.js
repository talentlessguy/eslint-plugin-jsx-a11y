...
function hasLabellingProp(openingElement: JSXOpeningElement, additionalLabellingProps?: Array<string> = []) {
  ...
  return openingElement.attributes.some((attribute): boolean => {
    ...
    // Attribute matches.
    if (labellingProps.includes(propName(attribute)) && !!getPropValue(attribute)) {
      return true
    }
    return false
  })
